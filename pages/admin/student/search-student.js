...
try{
  const resp = await fetch(
    `http://ec2-54-158-207-145.compute-1.amazonaws.com/api/v1/users/students/?code=${code.current.value}`,
    {
      method: 'GET',
      headers: {
...
const deleteHandler = async () => {
  ...
  try {
    const resp = await fetch(
      `http://ec2-54-158-207-145.compute-1.amazonaws.com/api/v1/users/students/${tobeDeleted._id}`,
      {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
...
const editHandler = async (e) => {
  ...
  try {
    const resp = await fetch(
      `http://ec2-54-158-207-145.compute-1.amazonaws.com/api/v1/users/students/${tobeEdited._id}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
      ...
      <div className="fixed overflow-hidden z-10 top-0 left-0 right-0 bottom-0  opacity-100 w-screen h-screen">
        ...
        <div className=" mt-16 ">
          <div className="p-4 m-auto max-w-sm bg-blue-200 rounded-xl  relative  ">
            <button onClick={deleteCancel} className="bg-red-500 text-white duration-200 transition-all hover:bg-red-600 px-2 rounded absolute top-4 right-4" >
            <i class="fa-solid fa-xmark"></i>
            </button>
            <p className="text-center font-Rubik font-bold text-gray-700 p-2 my-20">Are you sure you want to delete this student</p>
            <button onClick={deleteHandler} className="bg-red-500 text-white duration-200 transition-all hover:bg-red-600 p-2 rounded absolute bottom-4 right-4" >
              Confirm
            </button>
            </div>
          </div>
...
:null}
      ...
      <div className={`flex flex-row w-screen h-screen ${deleteModalIsOpen ? `bg-black opacity-60 overflow-hidden `:null} ${
