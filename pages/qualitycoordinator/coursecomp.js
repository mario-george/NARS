...
const coursecomp = ({ cookies }) => {
    ...
    useEffect(() => {
        ...
        const get_comp = async (e) => {
            ...
            try {
                const resp2 = await fetch(`${process.env.url}api/v1/programs/viewComp/${cookies.program}`, {
                    headers: {
                        Authorization: "Bearer " + cookies.token,
                    },
                });
    ...
    const handleSubmit = async (e) => {
        ...
        courses.map(async (course, index) => {
            ...
            checkboxRefs.current[index].forEach((isChecked, index) => {
                if (isChecked) {
                    comps.push({ "code": comp[index].code, "description": comp[index].description });
                }
            });
            try {
                const r = await fetch(`${process.env.url}api/v1/courses/original-courses/${course.id}`, {
                    method: "PATCH",
